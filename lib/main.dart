...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
      ...
      case AppLifecycleState.paused:
        ...
        Future<int>.value(1).then((int value) async {
          await Future.delayed(const Duration(
              seconds:
                  1)); // this is the most important to reproduce this problem

          final String now = tz.TZDateTime.now(tz.local).toIso8601String();
          await sharedPreferences.setString('paused', now);

          // Above Future delay is a web API call, for example, to get upcoming appointments
          await _zonedScheduleNotification(
              title: 'background title', body: 'background body');
        });
        break;

      case AppLifecycleState.resumed:
        setState(() {
          paused = sharedPreferences.getString('paused');
        });
  ...
  Future<void> _zonedScheduleNotification({
    ...
    await flutterLocalNotificationsPlugin.zonedSchedule(
        0,
        title,
        '$body ${notificationTime.toIso8601String()}',
        notificationTime,
        const NotificationDetails(
            android: AndroidNotificationDetails(
